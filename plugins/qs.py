...
def translate():
  ...
  if request.method == "POST":
    ...
    if not host:
      return zbx.no_host(host_tag, query_string)
    for _key, _val in request.args.items():
      # Skip the host's name; we already know it
      if _key == host_tag:
        continue
      zbx.add_param(host, f"{key_prefix}{_key}", str(_val), clock, ns)
    zbx.push()
    return f"{plugin_name} Blueprint"
  # else request.method == 'GET', but that's implied if you made it this far:
# -- end translate()
