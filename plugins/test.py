...
from plugins.file import readlocal, writelocal
import time

''' Test
Last change: test read/write to local cache
'''
...
def translate():
  if request.method == "POST":
    ...
    writelocal(plugin_name, host, qs, posted, datadict, result)
    return f"{plugin_name} Blueprint [POST]"  # TODO: return meaningful data.
  # else it's a GET request
  host = request.args.get("host")  # Universal ?host=
  return readlocal(plugin_name, host)
# -- end translate()
